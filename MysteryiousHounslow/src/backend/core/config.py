...
class Settings(BaseSettings):
    ...
    ALLOWED_API_KEYS: List[str] = Field(
        default_factory=list, description="Allowed API keys"
    )
    ...
    DIRECTUS_URL: str = Field(description="Directus API endpoint URL")
    DIRECTUS_API_KEY: str = Field(description="Directus API key")
    ...
    XGBOOST_MODEL_PATH: str = Field(
        default="./models/xgboost", description="XGBoost model directory"
    )
    ...
    XGBOOST_COLSAMPLE_BYTREE: float = Field(
        default=0.8, description="Column sample ratio"
    )
    ...
    PYGAD_PARENT_SELECTION_TYPE: str = Field(
        default="tournament", description="Parent selection type"
    )
    PYGAD_CROSSOVER_TYPE: str = Field(
        default="single_point", description="Crossover type"
    )
    PYGAD_MUTATION_TYPE: str = Field(default="random", description="Mutation type")
    PYGAD_MUTATION_PERCENT_GENES: float = Field(
        default=10.0, description="Mutation percentage"
    )

    # OR-Tools Configuration
    ORTOOLS_MAX_TIME: int = Field(
        default=30, description="Maximum solver time in seconds"
    )
    ORTOOLS_NUM_SEARCH_WORKERS: int = Field(
        default=4, description="Number of search workers"
    )
    ORTOOLS_ENABLE_LNS: bool = Field(
        default=True, description="Enable Large Neighborhood Search"
    )
    ...
    MICROTASK_RETRY_ATTEMPTS: int = Field(
        default=3, description="Microtask retry attempts"
    )
    MICROTASK_RETRY_DELAY: int = Field(
        default=1, description="Microtask retry delay in seconds"
    )

    # Constraint Configuration
    CONSTRAINT_VALIDATION_ENABLED: bool = Field(
        default=True, description="Enable constraint validation"
    )
    SPATIAL_CONSTRAINT_INDEX: bool = Field(
        default=True, description="Use spatial indexes"
    )
