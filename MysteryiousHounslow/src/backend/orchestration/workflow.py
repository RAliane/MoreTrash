...
from src.backend.orchestration.microtasks import MicrotaskExecutor
from src.backend.orchestration.pipeline import (
...
class OptimizationWorkflow:
        ...
        try:
            ...
            for stage in self.stages:
                ...
                if context.metadata.get("cancelled", False):
                    self.logger.info(
                        "Workflow cancelled", extra={"request_id": request_id}
                    )
                    break
            ...
            response = OptimizationResponse(
                ...
                constraint_satisfaction=context.metadata.get(
    ...
    async def cancel(
        ...
        """
        self.logger.info(
            "Cancelling optimization workflow", extra={"request_id": request_id}
        )
        ...
        self.logger.info(
            "Optimization workflow cancelled", extra={"request_id": request_id}
        )
    ...
    async def get_status(self, request_id: str) -> Dict[str, Any]:
        ...
        completed_stages = sum(
            1 for stage in self.stages if stage.is_completed(request_id)
        )
...
class BatchOptimizationWorkflow:
    ...
    async def execute_batch(
        ...
        # Create tasks
        tasks = [
            process_single_request(request, idx) for idx, request in enumerate(requests)
        ]
        ...
        self.logger.info(
            ...
            extra={
                ...
                "num_requests": len(requests),
                "successful": sum(
                    1 for r in responses if r.status == OptimizationStatus.COMPLETED
                ),
                "failed": sum(
                    1 for r in responses if r.status == OptimizationStatus.FAILED
                ),
            },
        )
...
class WorkflowMonitor:
    ...
    def start_workflow(self, request_id: str, request: OptimizationRequest) -> None:
        ...
        self.logger.info(
            "Workflow monitoring started", extra={"request_id": request_id}
        )
